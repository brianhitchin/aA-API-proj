        ...
        if (name && description && type && price && startDate && endDate) {
            ...
            if (newstartDate <= newendDate && description.length >= 30) {
                if (parseInt(price) !== 0) {
                    ...
                    return dispatch(eventsActions.create(thisgroup.id, { venueId: 1, capacity: 20, name, description, type, price: 0, startDate: new Date(startDate), endDate: new Date(endDate) }))
                        .then((res) => history.push(`/events/${res}`))
                        .catch(
                            async (res) => {
                                alert('error!')
